...
export async function POST(request: Request) {
  ...
  const prompt = `너는 학생 탐구 설계를 돕는 교육 연구 조력자다. Google Search로 최신·신뢰 가능한 자료를 찾아 아래 정보를 바탕으로 선택한 사고 형식에 맞는 탐구 주제와 보고서 초안을 작성하라.\n\n관심 주제 또는 개념: ${body.interest}\n선택한 사고 형식: ${body.perspectives.join(", ")}\n선택한 탐구 경로: ${body.path || "자료 분석형"}\n\n규칙: 학생이 직접 관찰·조사·비교·검증할 수 있는 범위로 구체화하고, 선택한 사고 형식이 제목·질문·보고서 내용에 드러나야 한다. 기본 탐구는 관찰과 자료 수집 중심, 심화 탐구는 조건 비교·한계·확장 질문 중심으로 작성한다. 검색한 자료는 출처명과 URL을 추천 자료에 포함한다. 확인되지 않은 사실을 단정하지 않는다. 반드시 아래 JSON 객체만 반환한다.\n{\"topic\":\"발전된 탐구 주제 제목\",\"question\":\"핵심 탐구 질문\",\"basic\":\"기본 탐구 내용 2~3문장\",\"advanced\":\"심화 탐구 내용 2~3문장\",\"recommendedMaterials\":\"추천 자료명·출처·URL과 기록 방법\",\"reportSections\":[{\"title\":\"탐구 동기와 질문\",\"prompt\":\"이 주제를 선택한 이유와 핵심 질문\",\"body\":\"보고서 초안\"},{\"title\":\"배경 지식과 자료 조사\",\"prompt\":\"핵심 개념과 참고 자료\",\"body\":\"보고서 초안\"},{\"title\":\"탐구 방법\",\"prompt\":\"관찰·조사·검증 절차\",\"body\":\"보고서 초안\"},{\"title\":\"탐구 결과와 해석\",\"prompt\":\"수집한 자료의 정리와 해석\",\"body\":\"보고서 초안\"},{\"title\":\"결론과 제안\",\"prompt\":\"알게 된 점과 후속 질문\",\"body\":\"보고서 초안\"}]}`;
  const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent`, { method: "POST", headers: { "Content-Type": "application/json", "x-goog-api-key": body.apiKey }, body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: prompt }] }], tools: [{ google_search: {} }], generationConfig: { temperature: 0.35, responseMimeType: "application/json" } }) });
