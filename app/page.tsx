...
export default function Home() {
  ...
  async function generateWithGoogle() {
    ...
    } catch (error) { const message = error instanceof Error ? error.message : "생성 중 오류가 발생했습니다."; setAiStatus(message); if (!key) setSettings(true); }
    finally { setAiLoading(false); }
  }

  return <main className="inquiry-app"><button className="api-settings-fab" onClick={() => setSettings(true)}>⚙ Google API 설정</button>{step >= 2 && step <= 5 && <div className="google-inquiry-panel"><div><strong>Google Gemini로 탐구 설계 전체 생성</strong><small>관점별 예시부터 탐구 경로, 기본·심화 내용, 보고서 초안까지 한 번에 반영합니다.</small></div><button className="ai-button" onClick={generateWithGoogle} disabled={aiLoading}>{aiLoading ? "Gemini 생성 중..." : aiContent ? "↻ Gemini 결과 갱신" : "✦ Gemini로 전체 생성"}</button>{aiStatus && <span className="ai-status-inline">{aiStatus}</span>}{aiContent && <div className="ai-result"><b>{aiContent.topic}</b><strong>Gemini 핵심 질문</strong><p>{aiContent.question}</p><strong>기본 탐구</strong><p>{aiContent.basic}</p><strong>심화 탐구</strong><p>{aiContent.advanced}</p><strong>추천 자료</strong><p>{aiContent.recommendedMaterials}</p>{aiContent.pathGuide && <><strong>선택 경로 안내</strong><p>{aiContent.pathGuide}</p></>}</div>}</div>}
    ...
    <section className="inquiry-content">
      ...
      {step === 2 && <section className="step-panel"><div className="selection-summary"><span>선택한 사고 형식</span><strong>{selectedNames.length ? selectedNames.join(" · ") : "아직 선택하지 않았어요"}</strong></div><div className="perspective-grid">{perspectives.map((p) => <button key={p.id} className={`perspective-card ${p.tint} ${selectedPerspectives.includes(p.id) ? "selected" : ""}`} onClick={() => togglePerspective(p.id)}><span className="perspective-icon">{p.icon}</span><span className="card-check">{selectedPerspectives.includes(p.id) ? "✓" : ""}</span><strong>{p.title}</strong><small>{p.desc}</small><em>{p.question}</em></button>)}</div><div className="example-preview"><div className="example-preview-head"><span>✦</span><div><strong>{aiContent ? "Gemini가 생성한 10가지 관점별 예시" : "관심 주제로 미리 보는 10가지 관점 예시"}</strong><small>{aiContent ? "1단계 키워드를 10가지 사고 형식에 적용한 결과입니다. 선택할 내용을 골라보세요." : "Gemini 전체 생성을 누르면 10가지 관점별 예시가 생성됩니다."}</small></div></div><div className="example-list">{perspectives.map((p) => <div className={`example-item ${selectedPerspectives.includes(p.id) ? "is-selected" : ""}`} key={p.id}><span className={`example-mark ${p.tint}`}>{p.title}</span><p>{aiContent?.perspectiveExamples?.[p.id] || perspectiveExample(interest, p)}</p></div>)}</div></div></section>}
      {step === 3 && <section className="step-panel"><div className="path-list">{paths.map((path) => <button key={path.id} className={`path-card ${selectedPath === path.id ? "selected" : ""}`} onClick={() => setSelectedPath(path.id)}><span className="path-icon">{path.icon}</span><span className="path-copy"><strong>{path.title}</strong><small>{aiContent?.pathExamples?.[path.id] || path.desc}</small></span><span className="path-time">예상 {path.time}</span><span className="radio">{selectedPath === path.id ? "✓" : ""}</span></button>)}</div><div className="path-preview"><div><span className="preview-label">{aiContent ? "Gemini가 설계한 선택 경로" : "선택한 경로"}</span><h3>{paths.find((p) => p.id === selectedPath)?.title}</h3><p>{aiContent?.pathExamples?.[selectedPath] || paths.find((p) => p.id === selectedPath)?.desc}</p></div><span className="preview-arrow">→</span></div></section>}
      {step === 4 && <section className="step-panel content-panel"><div className="topic-banner"><span className="topic-spark">✦</span><div><small>관심 출발점: {interest}</small><h3>{activeTopic}</h3><div className="topic-tags">{selectedNames.map((name) => <span key={name}>{name}</span>)}<span>{pathTitle}</span></div></div><button onClick={() => setStep(5)}>주제 수정</button></div><div className="depth-tabs"><button className={activeDepth === "basic" ? "active" : ""} onClick={() => setActiveDepth("basic")}>기본 탐구 <small>핵심 질문과 관찰</small></button><button className={activeDepth === "advanced" ? "active" : ""} onClick={() => setActiveDepth("advanced")}>심화 탐구 <small>분석과 확장 질문</small></button></div><div className="content-check"><span className="check-badge">✓</span><div><strong>{activeDepth === "basic" ? "먼저 이 정도로 시작해보세요" : "여기서 한 단계 더 깊게"}</strong><p>{aiContent ? (activeDepth === "basic" ? aiContent.basic : aiContent.advanced) : inquiryContent(interest, primaryPerspective, activeDepth)}</p></div></div><div className="question-grid"><div><span>핵심 질문</span><strong>{aiContent?.question || `${interest}를 ${primaryPerspective?.title || "탐구"}의 관점에서 어떻게 확인할 수 있을까?`}</strong></div><div><span>추천 자료</span><strong>{aiContent?.recommendedMaterials || (activeDepth === "basic" ? "관찰 기록 · 학생 인터뷰 · 사용량 체크표" : "설문 통계 · 사례 비교 · 비용 계산표")}</strong></div></div></section>}
